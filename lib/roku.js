...
module.exports = {
  ...
  rokuQuery (resource, method, returnRaw, returnHeaders) {
    ...
    return Request({
      ...
      if (returnHeaders) {
        return parseXML(response.body).then((result) => {
  ...
  listApps () {
    return this.rokuQuery('query/apps').then((response) => {
      if (response.apps && response.apps.app) {
        ...
        let promiseList = []
        _.forEach(apps, (app) => {
          promiseList.push(this.rokuQuery(`query/icon/${app.id}`, null, true, true))
        })
        return Promise.all(promiseList).then((response) => {
          _.forEach(response, (iconData, index) => {
            let buffer = Buffer.alloc(parseInt(iconData.headers['content-length']), iconData.body)
            let icon = `data:${iconData.headers['content-type']};base64,${buffer.toString('base64')}`
